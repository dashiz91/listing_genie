...
export const CreatorView: React.FC<CreatorViewProps> = ({
  ...
  const handleImageFiles = async (files: FileList | null) => {
    ...
    // Collect all successful uploads first, then update state once
    // (avoids stale-closure bug where only last file survives)
    const newUploads: UploadWithPreview[] = [];

    for (const file of fileArray) {
      if (!file.type.startsWith('image/')) continue;
      if (file.size > 10 * 1024 * 1024) {
        setUploadError('Some files are too large (max 10MB)');
        continue;
      }
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 pb-24">
        ...
        <div className="mb-6">
          {uploads.length > 0 ? (
            <div className="space-y-3">
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                ...
                {uploads.length < maxImages && (
                  <label className={cn(
                    "flex flex-col items-center justify-center rounded-xl border-2 border-dashed cursor-pointer transition-colors aspect-square",
                    isUploading
                      ? "border-redd-500/50 bg-redd-500/5"
                      : "border-slate-600 hover:border-redd-500/50 bg-slate-800/30"
                  )}>
                    <input
                      type="file"
            ...
            <label className={cn(
              "flex flex-col items-center justify-center py-12 px-6 border-2 border-dashed rounded-2xl cursor-pointer transition-all",
              isUploading
                ? "border-redd-500/50 bg-redd-500/5"
                : "border-slate-600 hover:border-redd-500/50 bg-slate-800/30 hover:bg-slate-800/50"
            )}>
              <input
                type="file"
                accept="image/png,image/jpeg,image/jpg"
              ...
              {isUploading ? (
                <>
                  <Spinner size="lg" className="text-redd-500 mb-4" />
                  <span className="text-base text-slate-300 font-medium mb-1">Uploading...</span>
                </>
              ) : (
                <>
